...
namespace cl {
    struct sha256 {
        ...
        // Constants
        static IO_CONSTEXPR_VAR usize BLOCK_BYTES = 64;
        static IO_CONSTEXPR_VAR usize DIGEST_BYTES = 32;
        static IO_CONSTEXPR_VAR usize HMAC_DIGEST_BYTES = 32;
        ...
        // RAII
        explicit IO_CONSTEXPR sha256() noexcept :
            _data{ 0 },
    ...
    private:
        ...
        IO_NODISCARD IO_CONSTEXPR static u32 rotr(u32 a, u32 b) noexcept { return (a >> b) | (a << (32 - b)); }
        IO_NODISCARD IO_CONSTEXPR static u32 ch(u32 x, u32 y, u32 z) noexcept { return (x & y) ^ (~x & z); }
        IO_NODISCARD IO_CONSTEXPR static u32 major(u32 x, u32 y, u32 z) noexcept { return (x & y) ^ (x & z) ^ (y & z); }
        IO_NODISCARD IO_CONSTEXPR static u32 ep0(u32 x) noexcept { return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22); }
        IO_NODISCARD IO_CONSTEXPR static u32 ep1(u32 x) noexcept { return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25); }
        IO_NODISCARD IO_CONSTEXPR static u32 sig0(u32 x) noexcept { return rotr(x, 7) ^ rotr(x, 18) ^ (x >> 3); }
        IO_NODISCARD IO_CONSTEXPR static u32 sig1(u32 x) noexcept { return rotr(x, 17) ^ rotr(x, 19) ^ (x >> 10); }
    ...
    private:
        ...
        io::u64 _bit_length;       // Total length of the message in bits
        u32 _data_count;
    }; // struct sha256
    ...
    void sha256::hmac(byte_view key, byte_view data, byte_view_mut out) noexcept {
        ...
        if (key.size() > BLOCK_BYTES) {
            sha256 tmp;
            tmp.update(key);
            byte_view_mut out32{ io::as_view_mut(key_pad).v.first(32) }; // trunc to first 32 bytes
        ...
        { // ---- Step 3: inner = hash(i_pad || data) ----
            ...
            inner.update(data);
            inner.final_without_logical_reset(io::as_view_mut(inner_hash));
        }
        ...
        io::secure_zero(key_pad, sizeof(key_pad));
        io::secure_zero(i_pad, sizeof(i_pad));
        io::secure_zero(o_pad, sizeof(o_pad));
        io::secure_zero(inner_hash, sizeof(inner_hash));
    } // hmac
    ...
    // SHA-256 constants
    static IO_CONSTEXPR_VAR io::u32 SHA256_K[64]{
    ...
    void sha256::final_without_logical_reset(byte_view_mut out) noexcept {
        ...
        _bit_length += static_cast<io::u64>(_data_count) * 8;
