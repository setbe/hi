...
namespace cl {
    ...
    struct x25519 {
        ...
        // Constants
        static IO_CONSTEXPR_VAR usize SCALAR_BYTES = 32;
        static IO_CONSTEXPR_VAR usize PUBKEY_BYTES = 32;
        static IO_CONSTEXPR_VAR usize SHARED_BYTES = 32;
    ...
    private:
        ...
        IO_NODISCARD static IO_CONSTEXPR u64 load64_le(const u8* p) noexcept {
            return (u64)p[0]
                | ((u64)p[1] << 8)
                | ((u64)p[2] << 16)
                | ((u64)p[3] << 24)
        ...
        static IO_CONSTEXPR void store64_le(u8* p, u64 x) noexcept {
            p[0] = (u8)(x >> 0);  p[1] = (u8)(x >> 8);
            p[2] = (u8)(x >> 16); p[3] = (u8)(x >> 24);
            p[4] = (u8)(x >> 32); p[5] = (u8)(x >> 40);
            p[6] = (u8)(x >> 48); p[7] = (u8)(x >> 56);
        }

        static IO_CONSTEXPR void clamp_scalar(u8 s[32]) noexcept {
            s[0] &= 248u;
            s[31] &= 127u;
            s[31] |= 64u;
        }

        static IO_CONSTEXPR void fe_0(fe& x) noexcept { x.v[0] = x.v[1] = x.v[2] = x.v[3] = x.v[4] = 0; }
        static IO_CONSTEXPR void fe_1(fe& x) noexcept { fe_0(x); x.v[0] = 1; }

        static IO_CONSTEXPR void fe_copy(fe& d, const fe& a) noexcept {
            d.v[0] = a.v[0]; d.v[1] = a.v[1]; d.v[2] = a.v[2]; d.v[3] = a.v[3]; d.v[4] = a.v[4];
        }

        // (a+b) mod p in lazy form
        static IO_CONSTEXPR void fe_add(fe& out, const fe& a, const fe& b) noexcept {
            out.v[0] = a.v[0] + b.v[0];
            out.v[1] = a.v[1] + b.v[1];
            out.v[2] = a.v[2] + b.v[2];
            out.v[3] = a.v[3] + b.v[3];
        ...
        static IO_CONSTEXPR void fe_sub(fe& out, const fe& a, const fe& b) noexcept {
        ...
        static IO_CONSTEXPR void fe_carry(fe& x) noexcept {
        ...
        // 19*x = (16+2+1)*x
        IO_NODISCARD static inline u64 mul19_u64(u64 x) noexcept { return (x<<4) + (x<<1) + x; }
        // floor(a / 2^51)
        IO_NODISCARD static inline u64 shr_u128_51(u128 a) noexcept { return (a.lo>>51) | (a.hi << (64-51)); }
        IO_NODISCARD static inline u64 and_mask51(u128 a) noexcept { return a.lo & ((1ull << 51) - 1ull); }
        IO_NODISCARD static inline u128 madd(u128 acc, u64 x, u64 y) noexcept { return io::add_u128(acc, io::mul_u64(x, y)); };
        ...
        static IO_CONSTEXPR void fe_cswap(fe& a, fe& b, u64 swap) noexcept {
        ...
        static void fe_from_u(fe& out, const u8 in[32]) noexcept {
            ...
            const u64 x0 = load64_le(t + 0);
            const u64 x1 = load64_le(t + 8);
            const u64 x2 = load64_le(t + 16);
            const u64 x3 = load64_le(t + 24);
        ...
        static IO_CONSTEXPR u64 ct_ge(u64 a, u64 b) noexcept {
        ...
        static void scalar_mult_u(u8 out[32], const u8 scalar_in[32], const u8 u_in[32]) noexcept {
            ...
            io::secure_zero(e, sizeof(e));
        }

        IO_NODISCARD static IO_CONSTEXPR bool is_all_zero32(const u8 x[32]) noexcept {
            u8 acc = 0;
    ...
    public:
        ...
        template<usize N>
        IO_NODISCARD static IO_CONSTEXPR bool ct_equal32(byte_view_n<N> a, byte_view_n<N> b) noexcept {
            static_assert(N == 32, "x25519::ct_equal32: size must be 32");
            u8 diff = 0;
            for (usize i = 0; i < 32; ++i) diff |= (u8)(a.v[i] ^ b.v[i]);
            return diff == 0;
        }
        ...
        static void generate_public(byte_view_n<SCALAR_BYTES> priv32,
            ...
            io::secure_zero(priv, sizeof(priv));
        }

        // out = priv * peer_pub; returns false if shared secret is all-zero
        IO_NODISCARD static bool shared_secret(byte_view_n<SCALAR_BYTES> priv32,
        ...
        static void generate_public(const u8(&priv)[32], u8(&pub)[32]) noexcept {
            generate_public(io::as_view(priv), io::as_view_mut(pub));
        }
        IO_NODISCARD static bool shared_secret(const u8(&priv)[32], const u8(&peer_pub)[32], u8(&out)[32]) noexcept {
            return shared_secret(io::as_view(priv), io::as_view(peer_pub), io::as_view_mut(out));
        }
    };
