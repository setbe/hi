...
namespace cl {
    struct poly1305 {
        ...
        // Constants
        static IO_CONSTEXPR_VAR usize TAG_BYTES = 16;
        static IO_CONSTEXPR_VAR usize OTKEY_BYTES = 32; // r(16) || s(16)

        static_assert(TAG_BYTES == 16, "poly1305: tag is 16 bytes");
        static_assert(OTKEY_BYTES == 32, "poly1305: one-time key is 32 bytes");

        // RAII
        explicit IO_CONSTEXPR poly1305() noexcept
    ...
    private:
        ...
        IO_CONSTEXPR static void store32_le(u8* p, u32 v) noexcept {
            p[0]=(u8)(v>>0);  p[1]=(u8)(v>>8);
            p[2]=(u8)(v>>16); p[3]=(u8)(v>>24);
        }
        IO_CONSTEXPR static void clamp_r(u8 r[16]) noexcept {
        ...
        void init(byte_view otk32) noexcept {
            ...
            u32 t0 = load32_le(rbytes + 0);
            u32 t1 = load32_le(rbytes + 4);
            u32 t2 = load32_le(rbytes + 8);
            u32 t3 = load32_le(rbytes + 12);
            ...
            _s[0] = load32_le(otk32.data() + 16);
            _s[1] = load32_le(otk32.data() + 20);
            _s[2] = load32_le(otk32.data() + 24);
            _s[3] = load32_le(otk32.data() + 28);
    ...
    public:
        ...
        void wipe() noexcept {
            io::secure_zero(_h, sizeof(_h));
            io::secure_zero(_r, sizeof(_r));
            io::secure_zero(_r5, sizeof(_r5));
            io::secure_zero(_s, sizeof(_s));
            io::secure_zero(_buf, sizeof(_buf));
            io::secure_zero(&_buf_used, sizeof(_buf_used));
            _buf_used = 0;
        }
        ...
        template<usize N> void init(const u8(&otk)[N]) noexcept { init(io::as_view(otk)); }

        template<usize N> void update(io::byte_view_n<N> m) noexcept { update((io::byte_view)m.v); }
        template<usize N> void update(const u8(&m)[N]) noexcept { update(io::as_view(m)); }
        ...
        template<usize N>
        IO_NODISCARD static IO_CONSTEXPR bool ct_equal(byte_view_n<N> a, byte_view_n<N> b) noexcept {
            u8 diff = 0;
        ...
        static void derive_otk_from_chacha20(byte_view_n<NK> key,
            ...
            static_assert(NK == cl::chacha20::KEY_BYTES, "key must be 32 bytes");
            static_assert(NN == cl::chacha20::NONCE_BYTES, "nonce must be 12 bytes");
            cl::chacha20 c;
        ...
        static void derive_otk_from_chacha20(const u8(&key)[NK],
            ...
            derive_otk_from_chacha20(io::as_view(key), io::as_view(nonce), io::as_view_mut(out_otk));
        }
        ...
        static void mac_chacha20(byte_view_n<NK> key,
            ...
            u8 otk[OTKEY_BYTES]{};
            derive_otk_from_chacha20(key, nonce, io::as_view_mut(otk));
            poly1305 p;
        ...
        static void mac_chacha20(const u8(&key)[32], const u8(&nonce)[12],
            ...
            mac_chacha20(io::as_view(key), io::as_view(nonce), io::as_view(msg), io::as_view_mut(tag));
        }

        
        template<usize NK, usize NN, usize NM>
        IO_NODISCARD static bool verify_chacha20(byte_view_n<NK> key,
                                                byte_view_n<NN> nonce,
                                                byte_view_n<NM> msg,
                                                byte_view_n<TAG_BYTES> tag) noexcept {
            u8 got[TAG_BYTES]{};
            mac_chacha20(key, nonce, msg, io::as_view_mut(got));
            return ct_equal<TAG_BYTES>(io::as_view(got), tag);
        }

        template<usize NM>
        IO_NODISCARD static bool verify(byte_view_n<OTKEY_BYTES> otk,
                                        byte_view_n<NM> msg,
                                        byte_view_n<TAG_BYTES> tag) noexcept {
            u8 got[TAG_BYTES]{};
            poly1305 p;
        ...
        template<usize NM>
        IO_NODISCARD static bool verify(const u8(&otk)[OTKEY_BYTES],
                                        const u8(&msg)[NM],
