...
namespace cl {
    struct sha384 {
        ...
        // Constants
        static IO_CONSTEXPR_VAR usize BLOCK_BYTES = 128; // 1024-bit blocks
        static IO_CONSTEXPR_VAR usize DIGEST_BYTES = 48;  // 384-bit digest
        static IO_CONSTEXPR_VAR usize HMAC_DIGEST_BYTES = 48;
        ...
        // RAII
        explicit IO_CONSTEXPR sha384() noexcept :
            _data{ 0 },
            _state{
    ...
    private:
        ...
        IO_NODISCARD IO_CONSTEXPR static u64 rotr(u64 x, u64 r) noexcept { return (x >> r) | (x << (64 - r)); }
        IO_NODISCARD IO_CONSTEXPR static u64 ch(u64 x, u64 y, u64 z) noexcept { return (x & y) ^ (~x & z); }
        IO_NODISCARD IO_CONSTEXPR static u64 major(u64 x, u64 y, u64 z) noexcept { return (x & y) ^ (x & z) ^ (y & z); }

        IO_NODISCARD IO_CONSTEXPR static u64 ep0(u64 x) noexcept { return rotr(x, 28) ^ rotr(x, 34) ^ rotr(x, 39); }
        IO_NODISCARD IO_CONSTEXPR static u64 ep1(u64 x) noexcept { return rotr(x, 14) ^ rotr(x, 18) ^ rotr(x, 41); }
        IO_NODISCARD IO_CONSTEXPR static u64 sig0(u64 x) noexcept { return rotr(x, 1) ^ rotr(x, 8) ^ (x >> 7); }
        IO_NODISCARD IO_CONSTEXPR static u64 sig1(u64 x) noexcept { return rotr(x, 19) ^ rotr(x, 61) ^ (x >> 6); }

        IO_CONSTEXPR static void add_bits(u64& lo, u64& hi, u64 add) noexcept {
            u64 old = lo;
            lo += add;
            if (lo < old) ++hi;
        }
    ...
    static IO_CONSTEXPR_VAR io::u64 SHA384_K[80]{
    ...
    void sha384::hmac(byte_view key, byte_view data, byte_view_mut out) noexcept {
        ...
        if (key.size() > BLOCK_BYTES) {
            ...
            byte_view_mut out48{ io::as_view_mut(key_pad).v.first(DIGEST_BYTES) };
